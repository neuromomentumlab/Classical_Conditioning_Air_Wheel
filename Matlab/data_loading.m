...
%%
%% Get Video Pointers
vpfile = fullfile(animal(1).video.mp4.paws); vp = VideoReader(vpfile);
vffile = fullfile(animal(1).video.mp4.face); vf = VideoReader(vffile);
vfile = fullfile(animal(1).video.mp4.pupil); v = VideoReader(vfile);
